...
if ( !class_exists( 'cavada_megamenu' ) ) {
	...
	class cavada_megamenu {
		...
		public function cavada_megamenu() {
		 ...
		 */
		public function tp_menu_header() {
			if ( basename( $_SERVER['PHP_SELF'] ) == "nav-menus.php" ) {
				wp_enqueue_style( 'wp-color-picker' );
				wp_enqueue_style( 'cavada-mega-menu-backend', get_template_directory_uri() . '/inc/megamenu/css/backend.css', array() );
				wp_enqueue_style( 'cavada-mega-menu-frontend', get_template_directory_uri() . '/inc/megamenu/css/tp-mega-menu.css', array() );
				wp_enqueue_style( 'cavada-font-awesome', get_template_directory_uri() . '/assets/css/font-awesome.min.css', array() );
				wp_enqueue_script(
					'cavada-mega-menu-js', get_template_directory_uri() . '/inc/megamenu/js/tp-mega-menu.js', array(
					'jquery',
					'jquery-ui-sortable'
				), false, true
				);
		 ...
		 */
		public function modify_arguments( $arguments ) {
		...
		public function update_menu( $menu_id, $menu_item_db ) {
			$check = apply_filters(
				'avf_mega_menu_post_meta_fields', array(
				'megamenu',
				'item-style',
				'menu-icon',
				'bg-color',
		...
		public function end_el( &$output, $item, $depth = 0, $args = array() ) {
			...
			if ( $submenu_type && $submenu_type == 'widget_area' ) {
				...
				if ( $content ) {
					...
					$output .= '<div class="sub-menu submenu-widget megacol">' . "\n" . $content . '</div>';
	...
	class tp_backend_walker extends Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
			...
			if ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
				...
				$title = sprintf( esc_html__( '%s (Pending)', 'cavada' ), $item->title );
			}
		...
		?>">
			...
			<dl class="menu-item-bar">
				<dt class="menu-item-handle">
					<span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title, 'cavada' ); ?></span> <span class="is-submenu" <?php echo $submenu_text; ?>><?php esc_html( 'sub item', 'cavada' ); ?></span></span>
                     <span class="item-controls">
                        <span class="item-type item-type-default"><?php echo esc_html( $item->type_label, 'cavada' ); ?></span>
                         <span class="item-order">
							...
							?>" class="item-move-up"><abbr title="<?php esc_attr_e( 'Move up', 'cavada' ); ?>">&#8593;</abbr></a>
                            |
                            <a href="<?php
							echo wp_nonce_url(
								add_query_arg(
							...
							?>" class="item-move-down"><abbr title="<?php esc_attr_e( 'Move down', 'cavada' ); ?>">&#8595;</abbr></a>
                        </span>
                        <a class="item-edit" id="edit-<?php echo esc_attr( $item_id ); ?>" title="<?php esc_attr_e( 'Edit Menu Item', 'cavada' ); ?>" href="<?php
						...
						?>"><?php esc_attr_e( 'Edit Menu Item', 'cavada' ); ?></a>
                    </span>
				</dt>
			</dl>
			...
			<div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr( $item_id ); ?>">
				...
				<p class="description description-thin description-label tp_label_desc_on_active">
					<label for="edit-menu-item-title-<?php echo esc_attr( $item_id ); ?>">
						<span class='tp_default_label'><?php esc_attr_e( 'Navigation Label', 'cavada' ); ?></span>
						<br />
				...
				<p class="description description-thin description-title">
					<label for="edit-menu-item-attr-title-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_attr_e( 'Title Attribute', 'cavada' ); ?><br />
				...
				<p class="field-link-target description description-thin">
					<label for="edit-menu-item-target-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_attr_e( 'link Target', 'cavada' ); ?><br />
						<select id="edit-menu-item-target-<?php echo esc_attr( $item_id ); ?>" class="widefat edit-menu-item-target" name="menu-item-target[<?php echo esc_attr( $item_id ); ?>]">
							<option value="" <?php selected( $item->target, '' ); ?>><?php esc_attr_e( 'Same window or tab', 'cavada' ); ?></option>
							<option value="_blank" <?php selected( $item->target, '_blank' ); ?>><?php esc_attr_e( 'New window or tab', 'cavada' ); ?></option>
						</select>
					</label>
				</p>
				<p class="field-css-classes description description-thin">
					<label for="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_attr_e( 'CSS Classes (optional)', 'cavada' ); ?><br />
				...
				<p class="field-xfn description description-thin">
					<label for="edit-menu-item-xfn-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_attr_e( 'link Relationship (XFN)', 'cavada' ); ?><br />
				...
				<p class="field-description description description-wide">
					<label for="edit-menu-item-description-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_attr_e( 'Description', 'cavada' ); ?><br />
				...
				<div class='tp_mega_menu_options'>
					...
					<p class="description description-wide tp_checkbox tp_mega_menu tp_mega_menu_d1">
						<label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
							...
							?>title="<?php esc_attr_e( 'Click to browse icon', 'cavada' ) ?>" <?php
								   ?>class="thickbox button-secondary submit-add-to-menu" <?php
								   ?>type="button" value="<?php esc_attr_e( 'Browse Icon', 'cavada' ) ?>"
					...
					<!-- ***************  end item *************** -->
					<?php
					$title = esc_html__( 'Hide text of This Item', 'cavada' );
					...
					<p class="description description-wide">
						<label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
							<input type="checkbox" value="active" id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo ent2ncr( $key . "[" . $item_id . "]" ); ?>" <?php echo esc_html( $value ); ?> /><?php echo esc_attr( $title ); ?>
						</label>
					</p>


					<?php
					$title = esc_html__( 'Disable Link', 'cavada' );
					...
					if ( !$depth ) {
						?>
						<?php
						$title              = esc_html__( 'Submenu Type', 'cavada' );
						...
						<p class="description description-wide description_width_100">
							...
							<label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
								<select id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo ent2ncr( $key . "[" . $item_id . "]" ); ?>" onchange="onChangeSubmenuType(this);">
									<option value="standard" <?php echo ( $value == 'standard' ) ? ' selected="selected" ' : ''; ?>><?php esc_attr_e( 'Standard Dropdown', 'cavada' ); ?></option>
									<option value="multicolumn" <?php echo ( $value == 'multicolumn' ) ? ' selected="selected" ' : ''; ?>><?php esc_attr_e( 'Multicolumn Dropdown', 'cavada' ); ?></option>
									<option value="widget_area" <?php echo ( $value == 'widget_area' ) ? ' selected="selected" ' : ''; ?>><?php esc_attr_e( 'Widget area', 'cavada' ); ?></option>
								</select>
							</label>
						</p>

						<?php
						$title    = esc_html__( 'Widget Area', 'cavada' );
						...
						<p class="description description-wide description_width_100 el_widget_area"<?php echo ent2ncr( $style ); ?>>
							...
							<label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
								<select id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo ent2ncr( $key . "[" . $item_id . "]" ); ?>">
									<option value="" <?php echo ( $value == '' ) ? ' selected="selected" ' : ''; ?>><?php esc_attr_e( 'Select Widget Area', 'cavada' ); ?></option>
									<?php
						...
						<?php
						$title = esc_html__( 'Submenu Columns (Not For Standard Drops)', 'cavada' );
						...
						<?php
						$title = esc_html__( 'Side of Dropdown Elements', 'cavada' );
						...
						<p class="description description-wide description_width_100">
							...
							<label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
								<select id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo ent2ncr( $key . "[" . $item_id . "]" ); ?>">
									<option value="drop_to_left" <?php echo ( $value == 'drop_to_left' ) ? ' selected="selected" ' : ''; ?>><?php esc_attr_e( 'Drop To Left Side', 'cavada' ); ?></option>
									<option value="drop_to_right" <?php echo ( $value == 'drop_to_right' ) ? ' selected="selected" ' : ''; ?>><?php esc_attr_e( 'Drop To Right Side', 'cavada' ); ?></option>
									<option value="drop_to_center" <?php echo ( $value == 'drop_to_center' ) ? ' selected="selected" ' : ''; ?>><?php esc_attr_e( 'Drop To Center', 'cavada' ); ?></option>
								</select>
							</label>
						</p>

						<?php
						$title = esc_html__( 'Enable Full Width Dropdown ()', 'cavada' );
						...
						<?php
						$title = esc_html__( 'DropDown Background Color', 'cavada' );
						...
						<p class="description description-wide tp_checkbox tp_mega_menu tp_mega_menu_d2"<?php echo ent2ncr( $style ); ?>>
							<label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
								<span class='tp_long_desc'><?php echo esc_attr( $title ); ?></span><br />
						...
						<?php
						$title = esc_html__( 'DropDown Background Image', 'cavada' );
						...
						<p class="description description-wide tp_checkbox tp_mega_menu tp_mega_menu_d2"<?php echo ent2ncr( $style ); ?>>
							<label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
								...
								<button id="browse-edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>"
										...
										onclick="open_media_browser('edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>'); return false;"><?php esc_attr_e( 'Browse Image', 'cavada' ); ?></button>
							</label>
						</p>
				...
				<div class="menu-item-actions description-wide submitbox">
					<?php if ( 'custom' != $item->type ) : ?>
						<p class="link-to-original">
							<?php printf( wp_kses( 'Original: %s', 'cavada' ), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
						</p>
					<?php endif; ?>
					...
					?>"><?php esc_attr_e( 'Remove', 'cavada' ); ?></a> <span class="meta-sep"> | </span>
					<a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr( $item_id ); ?>" href="<?php echo esc_url(
						add_query_arg(
							array(
...
if ( !function_exists( 'tp_fallback_menu' ) ) {
	...
	function tp_fallback_menu() {
		...
		echo "<li $current><a href='" . esc_url( home_url( '/' ) ) . "'>" . esc_attr__( 'Home', 'cavada' ) . "</a></li>";
