...
if ( ! class_exists( 'CAVADA_Tax_Meta_Class' ) ) :
     ...
     */
			...
			if ( in_array( $taxnow, $this->_meta_box['pages'] ) ) {
				...
				wp_enqueue_style( 'tax-meta-clss', $plugin_path . '/css/Tax-meta-class.css', array(), '3022016'  );
		...
		public function insert_images() {
			...
			foreach ( $_POST['attachments'] as $attachment_id => $attachment ) {
				...
				$li .= "<a title='" . esc_html__( 'Delete this image', 'cavada' ) . "' class='at-delete-file' href='#' rel='{$nonce}|{$term_id}|{$id}|{$attachment_id}'><img src='" . $this->SelfPath . "/images/delete-16.png' alt='" . esc_html__( 'Delete', 'cavada' ) . "' /></a>";
		...
		public function wp_ajax_delete_image() {
			...
			if ( $ok ) {
				...
				echo json_encode( array( 'message' => esc_html__( 'Cannot delete file. Something\'s wrong.', 'cavada' ) ) );
				die();
			}
		}
		...
		public function check_field_date() {
			...
			if ( $this->has_field( 'date' ) && $this->is_edit_page() ) {
				...
				wp_enqueue_style( 'cavada-jquery-ui-css', 'http://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/themes/base/jquery-ui.css', array() );
				wp_enqueue_script( 'cavada-jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/jquery-ui.min.js', array( 'jquery' ), true );
			}
		...
		public function check_field_time() {
			...
			if ( $this->has_field( 'time' ) && $this->is_edit_page() ) {
				...
				// Enqueu JQuery UI, use proper version.
				wp_enqueue_style( 'cavada-jquery-ui-css', 'https://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/themes/base/jquery-ui.css', array(), false, true );
				wp_enqueue_script( 'cavada--jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/jquery-ui.min.js', array( 'jquery' ), false, true );
				wp_enqueue_script( 'cavada-at-timepicker', 'https://github.com/trentrichardson/jQuery-Timepicker-Addon/raw/master/jquery-ui-timepicker-addon.js', array( 'tmc-jquery-ui' ), false, true );
        ...
        <img src="';
					...
					echo '" alt="' . esc_html__( 'Remove', 'cavada' ) . '" title="' . esc_html__( 'Remove', 'cavada' ) . '" id="remove-' . $field['id'] . '"></div>';
					$c = $c + 1;
			...
			echo '" alt="' . esc_html__( 'Add', 'cavada' ) . '" title="' . esc_html__( 'Add', 'cavada' ) . '" id="add-' . $field['id'] . '"><br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
		...
		public function show_field_file( $field, $meta ) {
			...
			if ( ! empty( $meta ) && count( $meta ) > 0 && ! $this->is_array_empty( $meta ) ) {
				...
				echo '<div style="margin-bottom: 10px"><strong>' . esc_html__( 'Uploaded files', 'cavada' ) . '</strong></div>';
				echo '<ol class="at-upload">';
				foreach ( $meta as $att ) {
					// if (wp_attachment_is_image($att)) continue; // what's image uploader for?
					echo "<li>" . wp_get_attachment_link( $att, '', false, false, ' ' ) . " (<a class='at-delete-file' href='#' rel='{$nonce}||{$field['id']}|{$att}'>" . esc_html__( 'Delete', 'cavada' ) . "</a>)</li>";
				}
				echo '</ol>';
			}
      ...
      <strong>" . esc_html__( 'Upload new files', 'cavada' ) . "</strong>\n
    </div>\n";
			echo "<div class='new-files'>\n
      <div class='file-input'>\n
        <input type='file' name='{$field['id']}[]' />\n
      </div><!-- End .file-input -->\n
      <a class='at-add-file button' href='#'>" . esc_html__( 'Add more files', 'cavada' ) . "</a>\n
      </div><!-- End .new-files -->\n";
			echo "</td>";
			$this->show_field_end( $field, $meta );
		}
		...
		public function show_field_color( $field, $meta ) {
			...
			echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . esc_html__( 'Select a color', 'cavada' ) . "'/>";
...
//get term meta field
if ( ! function_exists( 'cavada_get_tax_meta' ) ) {
	function cavada_get_tax_meta( $term_id, $key, $multi = false ) {
		$t_id = ( is_object( $term_id ) ) ? $term_id->term_id : $term_id;
		$m    = get_option( 'tax_meta_' . $t_id );
		if ( isset( $m[$key] ) ) {
			return $m[$key];
...
//delete meta
if ( ! function_exists( 'cavada_delete_tax_meta' ) ) {
	function cavada_delete_tax_meta( $term_id, $key ) {
		$m = get_option( 'tax_meta_' . $term_id );
		if ( isset( $m[$key] ) ) {
			unset( $m[$key] );
		}
...
//update meta
if ( ! function_exists( 'cavada_update_tax_meta' ) ) {
	function cavada_update_tax_meta( $term_id, $key, $value ) {
		$m       = get_option( 'tax_meta_' . $term_id );
		$m[$key] = $value;
		update_option( 'tax_meta_' . $term_id, $m );
	}
...
if ( ! function_exists( 'cavada_get_tax_meta_strip' ) ) {
	function cavada_get_tax_meta_strip( $term_id, $key, $multi = false ) {
		$t_id = ( is_object( $term_id ) ) ? $term_id->term_id : $term_id;
		$m    = get_option( 'tax_meta_' . $t_id );
		if ( isset( $m[$key] ) ) {
