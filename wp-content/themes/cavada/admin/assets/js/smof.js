...
jQuery(document).ready(function ($) {
	...
	//var $fold = '.f_' + $('.of-radio-tile-img').data('id');
	//console.log($fold);
	function header_style_select() {
		if ($('.of-radio-img-selected').data('id') == 'header_v1') {
			jQuery('.f_header_v1').slideDown('normal', "swing");
		} else {
			jQuery('.f_header_v1').slideUp('normal', "swing");
		}

		if ($('.of-radio-img-selected').data('id') == 'header_v2') {
			jQuery('.f_header_v2').slideDown('normal', "swing");
		} else {
			jQuery('.f_header_v2').slideUp('normal', "swing");
		}

		if ($('.of-radio-img-selected').data('id') == 'header_v3') {
			jQuery('.f_header_v3').slideDown('normal', "swing");
		} else {
			jQuery('.f_header_v3').slideUp('normal', "swing");
		}
	};
	header_style_select();
	...
	// Process images option (header options)
	$('.of-radio-img-img-header').each(function () {
		$(this).hover(function () {
			...
			if ($next_image.css('z-index') != '100') {
				...
				if ($next_image.width() > $next_image.height()) {
					$next_image.css('top', $(this).parent().height() + 'px');
					if (line_index < 3) {
	...
	$('#of_generate_less').live('click', function () {
		...
		if (answer) {
			...
			$.post(ajaxurl, data, function (response) {
				...
				var fail_popup = $('#of-popup-fail');
				var success_popup = $('#of-popup-generate-less');
	...
	$('.select_show_or_hide select').on('change', function () {
		var data_show = $(this).data('show');
		if ($(this).val() == data_show) {
			$('.f_' + data_show).slideDown('normal', "swing");
		}
		else {
			$('.f_' + data_show).slideUp('normal', "swing");
		}
	}).trigger('change');
}); //end doc ready
