 ...
 * @since 1.4.0
 *
 * @param $cavada_data Options array
 *
 * @return array
 */
function of_filter_save_media_upload( $cavada_data ) {

	if ( ! is_array( $cavada_data ) ) {
		return $cavada_data;
	}

	foreach ( $cavada_data as $key => $value ) {
		if ( is_string( $value ) ) {
			$cavada_data[$key] = str_replace(
				array(
					site_url( '', 'http' ),
					site_url( '', 'https' ),
				), array(
 ...
 * @return array
 */
function of_filter_load_media_upload( $cavada_data ) {
