...
class Options_Machine {
	...
	public static function optionsframework_machine( $options ) {
		...
		do_action(
			'optionsframework_machine_before', array(
				'options'   => $options,
				'smof_data' => $smof_data,
			)
		);
		...
		foreach ( $options as $value ) {
			...
			/* condition start */
			if ( ! empty( $smof_data ) || ! empty( $cavada_data ) ) {
				...
				if ( $value['type'] != "heading" ) {
					...
					if ( array_key_exists( "fold", $value ) ) {
						if ( ( $value['type'] == "select" || $value['type'] == "select_google_font" || $value['type'] == "image" ) && isset( $value['position'] ) && $value['position'] != '' ) {
				...
				switch ( $value['type'] ) {
					...
					case 'select':
						...
						if (  isset( $value['data_show'] ) ) {
							$data_show = ' data-show = "'.$value['data_show'].'"';
						}
//						if ( $value['mod'] == 'mini' ) {
//							$mini = 'mini';
//						}else{
...
//						}
						...
						$output .= '<select class="select of-input" name="' . $value['id'] . '" id="' . $value['id'] . '"'.$data_show.'>';
					...
					case 'images':
						...
						if ( $value['id'] == 'header_style' ) {
							...
							foreach ( $value['options'] as $key => $option ) {
								...
								$image_thumb       = substr_replace( $option, '_thumb', $image_thumb_index, 0 );
								$output .= '<div class="image-box">';
								$output .= '<span>';
								$output .= '<input type="radio" id="of-radio-img-' . $value['id'] . $i . '" class="checkbox of-radio-img-radio" value="' . $key . '" name="' . $value['id'] . '" ' . $checked . ' />';
								$output .= '<div class="of-radio-img-label">' . $key . '</div>';
								$output .= '<img src="' . $image_thumb . '" alt="" class="of-radio-img-img-header ' . $selected . '" onClick="document.getElementById(\'of-radio-img-' . $value['id'] . $i . '\').checked = true;" rel-data="' . ( ( $i - 1 ) % 6 ) . '" data-id="' . $key . '"/>';
								$output .= '<img src="' . $option . '" alt="" style="display:none;position:absolute;" class="header_preview header_preview_' . $value['id'] . $i . '"/>';
								$output .= '</span>';
								$output .= '</div>';
							}
						} else {
					...
					case "info":
						$info_text = $value['std'];
						$output .= '<div class="of-info"><h3>' . $info_text . '</h3></div>';
						break;
					...
					case 'sorter':
						...
						$sortlists = isset( $cavada_data[$value['id']] ) && ! empty( $cavada_data[$value['id']] ) ? $cavada_data[$value['id']] : $value['std'];
					...
					case 'backup':
						...
						$backup       = of_get_options( CAVADA_BACKUPS );
						$init         = of_get_options( 'smof_init' );


						if ( ! isset( $backup['backup_log'] ) ) {
							$log = esc_html__( 'No backups yet', 'cavada' );
						} else {
						...
						$output .= '<p><strong>' . esc_html__( 'Last Backup : ', 'cavada' ) . '<span class="backup-log">' . $log . '</span></strong></p></div>' . "\n";
						$output .= '<a href="#" id="of_backup_button" class="button" title="Backup Options">' . esc_html__( 'Backup Options', 'cavada' ) . '</a>';
						$output .= '<a href="#" id="of_restore_button" class="button" title="Restore Options">' . esc_html__( 'Restore Options', 'cavada' ) . '</a>';
						$output .= '</div>';
					...
					case 'transfer':
						...
						$output .= '<a href="#" id="of_import_button" class="button" title="Restore Options">' . esc_html__( 'Import Options', 'cavada' ) . '</a>';
					...
					case 'list_information':
						...
						$output .= '<div class="table"><table border="0" cellpadding="3" cellspacing="2">';

						foreach ( $value['options'] as $select_ID => $option ) {
							$theValue = $option;
							$output .= '<tr><td width="25%">' . $select_ID . '</td><td>' . $theValue . '</td></tr>';
						}
						$output .= '</table></div>';
						break;
				}
				...
				do_action(
					'optionsframework_machine_loop', array(
						'options'   => $options,
						'smof_data' => $smof_data,
						'defaults'  => $defaults,
						'counter'   => $counter,
		...
		do_action(
			'optionsframework_machine_after', array(
				'options'   => $options,
				'smof_data' => $smof_data,
				'defaults'  => $defaults,
				'counter'   => $counter,
	...
	public static function optionsframework_import_function( $id, $std, $mod ) {
		...
		$uploader = '';
		$uploader .= apply_filters( 'cavada_import_options', '' );
		$uploader .= '<span class="button tp-import-action">' . esc_html__( 'Intall demo data', 'cavada' ) . '</span>';
		$uploader .= '<span class="text_note">' . esc_html__( 'Take about 1-15 mins', 'cavada' ) . '</span>';

		$uploader .= '
			<div style="display:none;width: 345px" class="tp_process_bar">
				<div style="display: none;" class="tpimport_download">
					<span class="text_note tp_process_title">' . esc_html__( 'Download demo data package...', 'cavada' ) . '</span>
					<div class="meter">
						<span style="width: 0px"></span>
					</div>
					<div class="tp_process_messase" style="font-size: smaller;"></div>
				</div>
				<div style="display: none;" class="tpimport_extract">
					<span class="text_note">' . esc_html__( 'Extract demo data package...', 'cavada' ) . '</span>
					<div class="meter">
						<span style="width: 0px"></span>
					</div>
					<div class="tp_process_messase" style="font-size: smaller;"></div>
				</div>
				<div style="display: none;" class="tpimport_core">
					<span class="text_note">' . esc_html__( 'Import Pages, Ports, Categories..', 'cavada' ) . '</span>
					<div class="meter">
						<span style="width: 0px"></span>
					</div>
					<div class="tp_process_messase" style="font-size: smaller;"></div>
				</div>
				<div style="display: none;" class="tpimport_widgets">
					<span class="text_note">' . esc_html__( 'Add widgets...', 'cavada' ) . '</span>
					<div class="meter">
						<span style="width: 0px"></span>
					</div>
					<div class="tp_process_messase" style="font-size: smaller;"></div>
				</div>
				<div style="display: none;" class="tpimport_setting">
					<span class="text_note">' . esc_html__( 'Reset theme options...', 'cavada' ) . '</span>
					<div class="meter">
						<span style="width: 0px"></span>
					</div>
					<div class="tp_process_messase" style="font-size: smaller;"></div>
				</div>
				<div style="display: none;" class="tpimport_menus">
					<span class="text_note">' . esc_html__( 'Setup menus...', 'cavada' ) . '</span>
					<div class="meter">
						<span style="width: 0px"></span>
					</div>
					<div class="tp_process_messase" style="font-size: smaller;"></div>
				</div>
                              <div style="display: none;" class="tpimport_slider">
					<span class="text_note">' . esc_html__( 'Setup slider...', 'cavada' ) . '</span>
					<div class="meter">
						<span style="width: 0px"></span>
					</div>
					<span class="text_note">' . esc_html__( 'If import slider don\'t finish you can import manual', 'cavada' ) . '</span>
					<div class="tp_process_messase" style="font-size: smaller;"></div>
				</div>
