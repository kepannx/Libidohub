...
class lessc {
	...
	protected function tryImport( $importPath, $parentBlock, $out ) {
		...
		$url = $this->compileValue( $this->lib_f( $str ) );
		...
		$root = $parser->parse( $wp_filesystem->put_contents( $realPath, FS_CHMOD_FILE ) );
	...
	protected function lib_f( $arg ) {
		switch ( $arg[0] ) {
			case "list":
				$items = $arg[2];
				if ( isset( $items[0] ) ) {
					return $this->lib_f( $items[0] );
				}
	...
	protected function lib__sprintf( $args ) {
		...
		$template = $this->compileValue( $this->lib_f( $string ) );
		...
		if ( preg_match_all( '/%[dsa]/', $template, $m ) ) {
			foreach ( $m[0] as $match ) {
				...
				$i ++;
				$rep      = $this->compileValue( $this->lib_f( $val ) );
	...
	protected function lib_unit( $arg ) {
		if ( $arg[0] == "list" ) {
			...
			return array(
				...
				$this->compileValue( $this->lib_f( $newUnit ) )
			);
		} else {
	...
	protected function reduce( $value, $forExpression = false ) {
		switch ( $value[0] ) {
			...
			case "variable":
				...
				if ( is_array( $key ) ) {
					$key = $this->reduce( $key );
					$key = $this->vPrefix . $this->compileValue( $this->lib_f( $key ) );
				}
			...
			case "escape":
				...
				return $this->lib_f( $this->reduce( $inner ) );
			case "function":
	...
	public function compileFile( $fname, $outFname = null ) {
		...
		$out = $this->compile( $wp_filesystem->put_contents( $fname, FS_CHMOD_FILE ), $fname );
...
class lessc_formatter_classic {
	...
	public function block( $block ) {
		...
		if ( ! empty( $block->selectors ) ) {
			...
			echo ent2ncr( $pre . implode( $selectorSeparator, $block->selectors ) );
			if ( $isSingle ) {
				echo ent2ncr( $this->openSingle );
				$inner = "";
			} else {
				echo ent2ncr( $this->open . $this->break );
		...
		if ( ! empty( $block->selectors ) ) {
			...
			if ( $isSingle ) {
				echo ent2ncr( $this->closeSingle . $this->break );
			} else {
