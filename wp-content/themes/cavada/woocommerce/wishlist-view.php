...
<?php do_action( 'yith_wcwl_before_wishlist_form', $wishlist_meta ); ?>
	...
	<form id="yith-wcwl-form" action="<?php echo esc_url( YITH_WCWL()->get_wishlist_url( 'view' . ( $wishlist_meta['is_default'] != 1 ? '/' . $wishlist_meta['wishlist_token'] : '' ) ) ) ?>" method="post" class="woocommerce">
		...
		<table class="shop_table cart wishlist_table" data-pagination="<?php echo esc_attr( $pagination ) ?>" data-per-page="<?php echo esc_attr( $per_page ) ?>" data-page="<?php echo esc_attr( $current_page ) ?>" data-id="<?php echo ( is_user_logged_in() ) ? esc_attr( $wishlist_meta['ID'] ) : '' ?>" data-token="<?php echo ( ! empty( $wishlist_meta['wishlist_token'] ) && is_user_logged_in() ) ? esc_attr( $wishlist_meta['wishlist_token'] ) : '' ?>">
			...
			<tr>
				...
				<th class="product-name">
					<span class="nobr"><?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', esc_html__( 'Product Name', 'yit' ) ) ?></span>
				</th>
				...
				<?php if ( $show_price ) : ?>
                    ...
                    <span class="nobr">
                        <?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', esc_html__( 'Unit Price', 'yit' ) ) ?>
                    </span>
					</th>
				...
				<?php if ( $show_stock_status ) : ?>
                    ...
                    <span class="nobr">
                        <?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', esc_html__( 'Stock Status', 'yit' ) ) ?>
                    </span>
					</th>
						...
						<tr id="yith-wcwl-row-<?php echo esc_attr($item['prod_id']) ?>" data-row-id="<?php echo esc_attr($item['prod_id']) ?>">
							...
							<?php if ( $show_price ) : ?>
								<td class="product-price">
									...
									if ( is_a( $product, 'WC_Product_Bundle' ) ) {
										...
										echo apply_filters( 'yith_free_text', esc_html__( 'Free!', 'yit' ) );
									}
									?>
								</td>
							<?php endif ?>
							...
							<?php if ( $show_stock_status ) : ?>
								<td class="product-stock-status">
									...
									if ( $stock_status == 'out-of-stock' ) {
										$stock_status = "Out";
										echo '<span class="wishlist-out-of-stock">' . esc_html__( 'Out of Stock', 'yit' ) . '</span>';
									} else {
										$stock_status = "In";
										echo '<span class="wishlist-in-stock">' . esc_html__( 'In Stock', 'yit' ) . '</span>';
									}
									?>
								</td>
							<?php endif ?>
							...
							<?php if ( $show_last_column ): ?>
								<td class="product-add-to-cart">
									...
									if ( $show_dateadded && isset( $item['dateadded'] ) ):
										echo '<span class="dateadded">' . sprintf( esc_html__( 'Added on : %s', 'yit' ), date_i18n( get_option( 'date_format' ), strtotime( $item['dateadded'] ) ) ) . '</span>';
									endif;
									?>
									...
									<?php if ( $available_multi_wishlist && is_user_logged_in() && count( $users_wishlists ) > 1 && $move_to_another_wishlist ): ?>
										<select class="change-wishlist selectBox">
											<option value=""><?php esc_html_e( 'Move', 'yit' ) ?></option>
											<?php
											foreach ( $users_wishlists as $wl ):
												...
												<option value="<?php echo esc_attr( $wl['wishlist_token'] ) ?>">
													...
													if ( $wl['wishlist_privacy'] == 1 ) {
														$wl_privacy = esc_html__( 'Shared', 'yit' );
													} elseif ( $wl['wishlist_privacy'] == 2 ) {
														$wl_privacy = esc_html__( 'Private', 'yit' );
													} else {
														$wl_privacy = esc_html__( 'Public', 'yit' );
													}
									...
									<?php if ( $is_user_owner && $repeat_remove_button ): ?>
										<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove_from_wishlist button" title="<?php esc_html_e( 'Remove this product', 'yit' ) ?>"><?php esc_html_e( 'Remove', 'yit' ) ?></a>
									<?php endif; ?>
								</td>
							...
							<?php if ( $is_user_owner ): ?>
								<td class="product-remove">
									<div>
										<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove remove_from_wishlist" title="<?php esc_html_e( 'Remove this product', 'yit' ) ?>">&times;</a>
									</div>
								</td>
							<?php endif; ?>
						</tr>
					...
					<td colspan="<?php echo esc_attr( $column_count ) ?>">
						<?php if ( $show_cb ) : ?>
							<div class="custom-add-to-cart-button-cotaniner">
								...
								) ) ) ?>" class="button alt" id="custom_add_to_cart"><?php esc_html_e( 'Add the selected products to the cart', 'yit' ) ?></a>
							</div>
						<?php endif; ?>
						...
						<?php if ( is_user_logged_in() && $is_user_owner && $show_ask_estimate_button && $count > 0 ): ?>
							<div class="ask-an-estimate-button-container">
								<a href="<?php echo ( $additional_info ) ? '#ask_an_estimate_popup' : $ask_estimate_url ?>" class="btn button ask-an-estimate-button" <?php echo ( $additional_info ) ? 'data-rel="prettyPhoto[ask_an_estimate]"' : '' ?> >
									...
									<?php esc_html_e( 'Ask for an estimate', 'yit' ) ?>
								</a>
							</div>
						<?php endif; ?>
		...
		<?php if ( $wishlist_meta['is_default'] != 1 ): ?>
			<input type="hidden" value="<?php echo ent2ncr($wishlist_meta['wishlist_token']) ?>" name="wishlist_id" id="wishlist_id">
		<?php endif; ?>
