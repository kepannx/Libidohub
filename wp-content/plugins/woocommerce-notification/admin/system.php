...
class VI_WNOTIFICATION_Admin_System {
	...
	public function page_callback() { ?>
		...
		<table cellspacing="0" id="status" class="widefat">
			...
			<tr>
				<td data-export-label="<?php esc_html_e( 'Session Save Path', 'woocommerce-notification' ) ?>"><?php esc_html_e( 'Session Save Path', 'woocommerce-notification' ) ?></td>
				<td>
					<?php

					if ( @fopen( session_save_path() . '/test-log.log', 'a' ) ) {
						echo '<mark class="yes">&#10004; <code class="private">' . session_save_path() . '</code></mark> ';
					} else {
						printf( '<mark class="error">&#10005; ' . esc_html__( 'To allow session, please contact your server supporter or make permission <code>%s</code> writable', 'woocommerce-notification' ) . '</mark>', session_save_path() );
					}
					?>
