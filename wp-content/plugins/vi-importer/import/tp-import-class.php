...
class ob_wp_import extends WP_Import {
	...
	function set_options( $option_file ) {
		...
		if ( ! vi_importer_require() ) {
			$setting_data = unserialize( urldecode( file_get_contents( $option_file ) ) );
			update_option( "theme_mods_$theme_slug", $setting_data );
		} else {
			$setting_data = json_decode( file_get_contents( $option_file ), true );
			update_option( $theme_slug . "_options_data", $setting_data );
		}
	...
	function import_revslider() {
		...
		if ( $handle = opendir( $_SESSION['villatheme-demodata-dir'] . "/data/revslider" ) ) {
			$check = 0;
			while ( false !== ( $entry = readdir( $handle ) ) ) {
				if ( $entry != "." && $entry != ".." ) {
					$_FILES['import_file']['tmp_name'] = $_SESSION['villatheme-demodata-dir'] . "/data/revslider/" . $entry;
					if ( $data_rev == $entry ) {
						$check = 1;
						continue;
					}
	...
	function import_widgets( $import_array ) {
		...
		// IMPORT Widget Logic
		$widgets_logic_file = $_SESSION['villatheme-demodata-dir'] . '/data/widget/widget_logic_options.txt'; // widgets data file
		...
		if ( is_file( $widgets_logic_file ) ) {
			...
			if ( trim( array_shift( $import ) ) == "[START=WIDGET LOGIC OPTIONS]" && trim( array_pop( $import ) ) == "[STOP=WIDGET LOGIC OPTIONS]" ) {
				...
				$wl_options['msg'] = esc_html__( 'Success! Options file imported', 'vi-importer' );
			} else {
				$wl_options['msg'] = esc_html__( 'Invalid options file', 'vi-importer' );
			}
