...
abstract class viBase {
	...
	protected function getpath( $url ) {
		$url = apply_filters( 'vi_filter_cssjs_alter_url', $url );
		...
		} else if ( ( strpos( $url, '//' ) === false ) && ( strpos( $url, parse_url( VI_WP_SITE_URL, PHP_URL_HOST ) ) === false ) ) {
			$url = VI_WP_SITE_URL . $url;
		}

		// first check; hostname wp site should be hostname of url
		$thisHost = @parse_url( $url, PHP_URL_HOST );
		if ( $thisHost !== parse_url( VI_WP_SITE_URL, PHP_URL_HOST ) ) {
			/* 
			* first try to get all domains from WPML (if available)
			* then explicitely declare $this->cdn_url as OK as well
			* then apply own filter vi_filter_cssjs_multidomain takes an array of hostnames
			...
			*/
			...
			if ( ! empty( $multidomainsWPML ) ) {
				$multidomains = array_map( array( $this, "vi_getDomain" ), $multidomainsWPML );
			}
			...
			$multidomains = apply_filters( 'vi_filter_cssjs_multidomain', $multidomains );

			if ( ! empty( $multidomains ) ) {
				if ( in_array( $thisHost, $multidomains ) ) {
					$url = str_replace( $thisHost, parse_url( VI_WP_SITE_URL, PHP_URL_HOST ), $url );
				} else {
					return false;
				}
			} else {
		...
		$tmp_vi_root = preg_replace( '/https?/', '', VI_WP_ROOT_URL );
		$tmp_url     = preg_replace( '/https?/', '', $url );
		$path        = str_replace( $tmp_vi_root, '', $tmp_url );
		...
		$path = str_replace( '//', '/', VI_ROOT_DIR . $path );

		return $path;
	}

	// needed for WPML-filter
	protected function vi_getDomain( $in ) {
		return ( parse_url( $in, PHP_URL_HOST ) );
	}


	// logger
	protected function vi_logger( $logmsg, $appendHTML = true ) {
		if ( $appendHTML ) {
			$logmsg = "<!--noptimize--><!-- " . $logmsg . " --><!--/noptimize-->";
			$this->content .= $logmsg;
		} else {
			$logfile = WP_CONTENT_DIR . '/vi_log.txt';
			$logmsg .= "\n--\n";
	...
	protected function url_replace_cdn( $url ) {
		if ( ! empty( $this->cdn_url ) ) {
			...
			if ( ( substr( $url, 0, 1 ) === '/' ) && ( substr( $url, 1, 1 ) !== '/' ) ) {
				...
				$WPSiteBreakdown = parse_url( VI_WP_SITE_URL );
		...
		$url = apply_filters( 'vi_filter_base_replace_cdn', $url );
			...
			$out = preg_replace_callback(
				...
				create_function(
					...
					if (substr($filepath,-3,3)===".js") {
						...
						if (get_option("vi_js_trycatch")==="on") {
							$filecontent="try{".$filecontent."}catch(e){}";
						}
					} else if ((substr($filepath,-4,4)===".css")) {
						$filecontent=viStyles::fixurls($filepath,$filecontent);
					}
