...
class viStyles extends viBase {
	...
	public function read( $options ) {
		$noptimizeCSS = apply_filters( 'vi_filter_css_noptimize', false, $this->content );
		if ( $noptimizeCSS ) {
			return false;
		}

		$whitelistCSS = apply_filters( 'vi_filter_css_whitelist', '' );
		...
		$removableCSS = apply_filters( 'vi_filter_css_removables', $removableCSS );
		if ( ! empty( $removableCSS ) ) {
			$this->cssremovables = array_filter( array_map( 'trim', explode( ",", $removableCSS ) ) );
		}

		$this->cssinlinesize = apply_filters( 'vi_filter_css_inlinesize', 256 );

		// filter to "late inject minified CSS", default to true for now (it is faster)
		$this->inject_min_late = apply_filters( 'vi_filter_css_inject_min_late', true );

		// Remove everything that's not the header
		if ( apply_filters( 'vi_filter_css_justhead', $options['justhead'] ) == true ) {
		...
		// include inline?
		if ( apply_filters( 'vi_css_include_inline', $options['include_inline'] ) == true ) {
			$this->include_inline = true;
		}

		// what CSS shouldn't be vid
		$excludeCSS = $options['css_exclude'];
		$excludeCSS = apply_filters( 'vi_filter_css_exclude', $excludeCSS );
		if ( $excludeCSS !== "" ) {
		...
		$this->defer = apply_filters( 'vi_filter_css_defer', $this->defer );
		...
		$this->inline = apply_filters( 'vi_filter_css_inline', $this->inline );
		...
		if ( preg_match_all( '#(<style[^>]*>.*</style>)|(<link[^>]*stylesheet[^>]*>)#Usmi', $this->content, $matches ) ) {
			foreach ( $matches[0] as $tag ) {
				if ( $this->isremovable( $tag, $this->cssremovables ) ) {
					...
					$media = apply_filters( 'vi_filter_css_tagmedia', $media, $tag );
	...
	public function minify() {
		foreach ( $this->css as $group ) {
			...
			if ( preg_match( '#^INLINE;#', $css ) ) {
				...
				$tmpstyle = apply_filters( 'vi_css_individual_style', $css, "" );
				if ( has_filter( 'vi_css_individual_style' ) && ! empty( $tmpstyle ) ) {
					$css                   = $tmpstyle;
				...
				if ( $css !== false && file_exists( $css ) && is_readable( $css ) ) {
					...
					$tmpstyle = apply_filters( 'vi_css_individual_style', $css, $cssPath );
					if ( has_filter( 'vi_css_individual_style' ) && ! empty( $tmpstyle ) ) {
						$css                   = $tmpstyle;
						$this->alreadyminified = true;
				...
				foreach ( $matches[0] as $import ) {
					if ( $this->isremovable( $import, $this->cssremovables ) ) {
						...
						if ( file_exists( $path ) && is_readable( $path ) ) {
							...
							$tmpstyle = apply_filters( 'vi_css_individual_style', $code, "" );
							if ( has_filter( 'vi_css_individual_style' ) && ! empty( $tmpstyle ) ) {
								$code                  = $tmpstyle;
								$this->alreadyminified = true;
		...
		foreach ( $this->csscode as &$code ) {
			...
			if ( ( $this->datauris == true ) && ( function_exists( 'base64_encode' ) ) && ( is_array( $matches ) ) ) {
				foreach ( $matches[2] as $count => $quotedurl ) {
					...
					$datauri_max_size = (int) apply_filters( 'vi_filter_css_datauri_maxsize', $datauri_max_size );
					$datauri_exclude  = apply_filters( 'vi_filter_css_datauri_exclude', "" );
			...
			// Minify
			if ( ( $this->alreadyminified !== true ) && ( apply_filters( "vi_css_do_minify", true ) ) ) {
			...
			$tmp_code = apply_filters( 'vi_css_after_minify', $code );
			if ( ! empty( $tmp_code ) ) {
				$code = $tmp_code;
				unset( $tmp_code );
			}
	...
	public function cache() {
		if ( $this->datauris ) {
			...
			$md5         = md5( $this->mhtml );
			$cache       = new viCache( $md5, 'txt' );
			if ( ! $cache->check() ) {
				// Cache our images for IE
				$cache->cache( $this->mhtml, 'text/plain' );
			}
			$mhtml = VI_CACHE_URL . $cache->getname();
		}
		...
		foreach ( $this->csscode as $media => $code ) {
			...
			$cache = new viCache( $md5, 'css' );
			if ( ! $cache->check() ) {
				// Cache our code
				$cache->cache( $code, 'text/css' );
			}
			$this->url[ $media ] = VI_CACHE_URL . $cache->getname();
		}
	}
	...
	public function getcontent() {
		...
		$replaceTag = apply_filters( 'vi_filter_css_replacetag', $replaceTag );
		...
		if ( $this->inline == true ) {
			...
			if ( $this->defer == true ) {
				...
				$defer_inline_code = apply_filters( 'vi_filter_css_defer_inline', $defer_inline_code );
				if ( ! empty( $defer_inline_code ) ) {

					$iCssHash  = md5( $defer_inline_code );
					$iCssCache = new viCache( $iCssHash, 'css' );
					if ( $iCssCache->check() ) {
	...
	static function fixurls( $file, $code ) {
		$file = str_replace( VI_ROOT_DIR, '/', $file );
		...
		if ( preg_match_all( '#url\((?!data)(?!\#)(?!"\#)(.*)\)#Usi', $code, $matches ) ) {
			...
			foreach ( $matches[1] as $k => $url ) {
				...
				if ( substr( $url, 0, 1 ) == '/' || preg_match( '#^(https?://|ftp://|data:)#i', $url ) ) {
					...
					// relative URL
					$newurl = preg_replace( '/https?:/', '', str_replace( " ", "%20", VI_WP_ROOT_URL . str_replace( '//', '/', $dir . '/' . $url ) ) );
