...
class viScripts extends viBase {
	...
	public function read($options) {
		$noptimizeJS = apply_filters( 'vi_filter_js_noptimize', false, $this->content );
                if ($noptimizeJS) return false;

		// only optimize known good JS?
		$whitelistJS = apply_filters( 'vi_filter_js_whitelist', "" );
		if (!empty($whitelistJS)) {
			$this->whitelist = array_filter(array_map('trim',explode(",",$whitelistJS)));
		}

		// is there JS we should simply remove
		$removableJS = apply_filters( 'vi_filter_js_removables', '');
		if (!empty($removableJS)) {
			$this->jsremovables = array_filter(array_map('trim',explode(",",$removableJS)));
		}

		// only header?
		if( apply_filters('vi_filter_js_justhead',$options['justhead']) == true ) {
		...
		// include inline?
		if( apply_filters('vi_js_include_inline',$options['include_inline']) == true ) {
			$this->include_inline = true;
		}

		// filter to "late inject minified JS", default to true for now (it is faster)
		$this->inject_min_late = apply_filters('vi_filter_js_inject_min_late',true);

		// filters to override hardcoded do(nt)move(last) array contents (array in, array out!)
		$this->dontmove = apply_filters( 'vi_filter_js_dontmove', $this->dontmove );
		$this->domovelast = apply_filters( 'vi_filter_js_movelast', $this->domovelast );
		$this->domove = apply_filters( 'vi_filter_js_domove', $this->domove );

		// get extra exclusions settings or filter
		$excludeJS = $options['js_exclude'];
		$excludeJS = apply_filters( 'vi_filter_js_exclude', $excludeJS );
		if ($excludeJS!=="") {
	...
	public function minify() {
		foreach($this->scripts as $script) {
			if(preg_match('#^INLINE;#',$script)) {
				...
				$tmpscript = apply_filters( 'vi_js_individual_script', $script, "" );
				if ( has_filter('vi_js_individual_script') && !empty($tmpscript) ) {
					$script=$tmpscript;
				...
				if($script !== false && file_exists($script) && is_readable($script)) {
					...
					$tmpscriptsrc = apply_filters( 'vi_js_individual_script', $scriptsrc, $script );
					if ( has_filter('vi_js_individual_script') && !empty($tmpscriptsrc) ) {
		...
		$ccheck = new viCache($this->md5hash,'js');
		if($ccheck->check()) {
		...
		if ($this->alreadyminified!==true) {
		  if (class_exists('JSMin') && apply_filters( 'vi_js_do_minify' , true)) {
			if (@is_callable(array("JSMin","minify"))) {
				...
				$this->jscode = apply_filters( 'vi_js_after_minify', $this->jscode );
				return true;
			} else {
				$this->jscode = $this->inject_minified($this->jscode);
				return false;
	...
	public function cache()	{
		$cache = new viCache($this->md5hash,'js');
		if(!$cache->check()) {
			//Cache our code
			$cache->cache($this->jscode,'text/javascript');
		}
		$this->url = VI_CACHE_URL.$cache->getname();
	...
	public function getcontent() {
		...
		$defer = apply_filters( 'vi_filter_js_defer', $defer );
		$bodyreplacementpayload = '<script type="text/javascript" '.$defer.'src="'.$this->url.'"></script>';
		$bodyreplacementpayload = apply_filters('vi_filter_js_bodyreplacementpayload',$bodyreplacementpayload);

		$bodyreplacement = implode('',$this->move['first']);
		$bodyreplacement .= $bodyreplacementpayload;
		$bodyreplacement .= implode('',$this->move['last']);

		$replaceTag = apply_filters( 'vi_filter_js_replacetag', $replaceTag );
