...
class viCache {
	...
	public function __construct($md5,$ext='php') {
		$this->cachedir = VI_CACHE_DIR;
		$this->delayed = VI_CACHE_DELAY;
		$this->nogzip = VI_CACHE_NOGZIP;
		if($this->nogzip == false) {
			$this->filename = VI_CACHEFILE_PREFIX.$md5.'.php';
		} else {
			if (in_array($ext, array("js","css")))	 {
				$this->filename = $ext.'/'.VI_CACHEFILE_PREFIX.$md5.'.'.$ext;
			} else {
				$this->filename = '/'.VI_CACHEFILE_PREFIX.$md5.'.'.$ext;
			}
		}
	}
	...
	public function cache($code,$mime) {
		if($this->nogzip == false) {
			...
			$phpcode = file_get_contents(VI_PLUGIN_DIR.'/config/'.$file);
	...
	static function clearall() {
		...
		foreach (array("","js","css") as $scandirName) {
			$scan[$scandirName] = scandir(VI_CACHE_DIR.$scandirName);
		}
		
		// clear the cachedirs
		foreach ($scan as $scandirName=>$scanneddir) {
			$thisAoCacheDir=rtrim(VI_CACHE_DIR.$scandirName,"/")."/";
			foreach($scanneddir as $file) {
				if(!in_array($file,array('.','..')) && strpos($file,VI_CACHEFILE_PREFIX) !== false && is_file($thisAoCacheDir.$file)) {
					@unlink($thisAoCacheDir.$file);
				}
			}
		}

		@unlink(VI_CACHE_DIR."/.htaccess");
		delete_transient("vi_stats");
		...
		if(function_exists('wp_cache_clear_cache')) {
			...
			wp_schedule_single_event( time() + 1, 'vi_flush_pagecache' , array(time()));
		}

		if (!function_exists('vi_do_cachepurged_action')) {
			function vi_do_cachepurged_action() {
				do_action("vi_action_cachepurged");
			}
		}
		add_action("after_setup_theme","vi_do_cachepurged_action");

		return true;
	}

	static function stats()	{
		$AOstats=get_transient("vi_stats");
		...
		if (empty($AOstats)) {
			...
			foreach ($scan as $scandirName=>$scanneddir) {
				$thisAoCacheDir=rtrim(VI_CACHE_DIR.$scandirName,"/")."/";
				foreach($scanneddir as $file) {
					if(!in_array($file,array('.','..')) && strpos($file,VI_CACHEFILE_PREFIX) !== false) {
						if(is_file($thisAoCacheDir.$file)) {
							if(VI_CACHE_NOGZIP && (strpos($file,'.js') !== false || strpos($file,'.css') !== false || strpos($file,'.img') !== false || strpos($file,'.txt') !== false )) {
								$count++;
							} elseif(!VI_CACHE_NOGZIP && strpos($file,'.none') !== false) {
								$count++;
							}
			...
			$AOstats=array($count,$size,time());
                        if ($count>100) {
				set_transient("vi_stats",$AOstats,HOUR_IN_SECONDS);
                        }
		}
	...
	static function cacheavail() {
		if(!defined('VI_CACHE_DIR')) {
			// We didn't set a cache
			return false;
		}
		
		foreach (array("","js","css") as $checkDir) {
			if(!viCache::checkCacheDir(VI_CACHE_DIR.$checkDir)) {
				return false;
			}
		}
		
		/** write index.html here to avoid prying eyes */
		$indexFile=VI_CACHE_DIR.'/index.html';
		if(!is_file($indexFile)) {
			@file_put_contents($indexFile,'<html><body>Generated by <a href="http://wordpress.org/extend/plugins/vi/">Autoptimize</a></body></html>');
		}

		/** write .htaccess here to overrule wp_super_cache */
	        $htAccess=VI_CACHE_DIR.'/.htaccess';
	        if(!is_file($htAccess)) {
			/** 
			...
			} else if (is_multisite() || VI_CACHE_NOGZIP == false) {
